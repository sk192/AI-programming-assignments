...
class Node: 
	def __init__(self, data, f_val, level):
		self.data = data
		self.f_val = f_val
		self.level = level					# level indiacate g(n) value
...
class puzzle:
	...
	def h1f(self,start,goal):
		return self.huristic1(start.data, goal) + start.level				# here start.level is g(n)
	...
	def process1(self, initial, goal):							# Process to solve 8 or 15 puzzle problem using misplaced tile heuristic

		print("\n1) Heusristic1: misplaced tile method \n")
...
def main():
	...
	if n==3:
		print("\n8Puzzle problem using A* Search algorithm: \n")
	else: 
		print("\n15Puzzle problem using A* Search algorithm: \n")
